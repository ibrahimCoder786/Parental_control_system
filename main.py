...
def remove_marked_entries(hosts_path):
    ...
    try:
        ...
        if len(filtered_lines) < original_lines_count:
            ...
            st.info("ℹ️ No application-specific entries found to remove.")
...
def block_sites(hosts_path, sites_to_block):
    if not sites_to_block:
        st.warning("⚠️ No sites specified to block.")
        return True
        ...
        if os.path.exists(hosts_path):
             try:
                with open(hosts_path, 'r', encoding='utf-8-sig') as f:
                 ...
                 st.warning(f"⚠️ Could not read existing hosts file: {e}")
        ...
        for site in sites_to_block:
            ...
            if site and '/' not in site and '.' in site:
                ...
                if is_new:
                 ...
                 st.warning(f"⚠️ Skipping invalid entry: '{site}'")

        if not lines_to_add_final:
            if sites_to_block:
                 st.info("ℹ️ All specified valid sites are already blocked.")
            return True
...
def allow_sites(hosts_path, sites_to_allow, block_all=True):
    if not sites_to_allow:
        st.warning("⚠️ No sites specified to allow.")
        return True
    ...
    try:
        ...
        if not lines_to_add_final:
            st.info("ℹ️ No new sites to block in allow mode.")
            return True
    ...
    try:
        if system == "Windows":
            ...
            if "Successfully flushed" in result.stdout:
                 ...
                 message = f"⚠️ Windows DNS flush command ran, but output unexpected"

        elif system == "Darwin":
            manual_action = "sudo dscacheutil -flushcache; sudo killall -HUP mDNSResponder"
            message = "⚠️ On macOS, please run DNS flush manually in Terminal."
            success = True
             ...
             message = "⚠️ On Linux, please run appropriate DNS flush command manually."
             success = True

        else:
             message = f"ℹ️ DNS flush command not known for system: {system}"
             success = True
...
def main_app():
    ...
    with col1:
        st.title("🛡️ Parental Control Pro")
    ...
    with tab_control:
                ...
                if remaining_time > 0:
                    ...
                    if restore_success:
                        ...
                        if st.session_state.backup_file_path:
                             st.info(f"ℹ️ Last known backup file: `{st.session_state.backup_file_path}`")
        ...
        with st.container():
                ...
                if st.button(button_text, disabled=st.session_state.running, type="primary", key="start_control"):
                    ...
                    if st.session_state.allowed_mode:
                        ...
                        if not current_sites_list:
                            st.warning("⚠️ Please enter at least one valid domain to allow in the Settings tab.")
                            ...
                            st.info(f"ℹ️ Using hosts file: `{hosts_file}`")
                        ...
                        if not current_sites_list:
                            st.warning("⚠️ Please enter at least one valid domain to block in the Settings tab.")
            ...
            with col2:
                button_text = "⏹️ Stop Control" if st.session_state.running else "Not Running"
    ...
    with tab_info:
        with st.container():
            st.markdown("<div class='custom-card'>", unsafe_allow_html=True)
            st.header("ℹ️ Application Info & Help")
            ...
            **Key Features**:
            ...
            - ⏱️ Timer-based control with automatic restoration
            ...
            st.warning("""
            ⚠️ **Note**: This is a focus tool, not comprehensive parental control software.
